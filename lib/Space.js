...
function mergeState(space, mergeObj, actionName) {
  ...
  if (mergeObj === null && space.key) {
    const match = space.name.match(listNameRegex);
    ...
    const list = space.nextParent.preState[listName];
...
Space.prototype.subSpace = function space(nameOrState, id) {
  ...
  const name = hasArray(this) ? 'root' : nameOrState;
  const parentArr = hasArray(this) ? this.preState : this.preState[name];
  const index = id && findIndexById(parentArr, id);
  if (index === -1) {
    throw new Error(
      'Could not find item with id ' + id + ' in ' + this.name + '.' + name
    );
  }
  const item = id ? parentArr[index] : this.preState[name];
  let subSpace;
  ...
  if (item instanceof Space) {
    ...
    if (id) {
      defineKeyGetter(subSpace);
      parentArr[index] = subSpace;
    } else {
