...
function setState(space, mergeObj, actionName) {
...
Space.prototype.doAction = function doAction(action, actionName) {
  const self = this;

  if (typeof action === 'object') {
    const mergeObj = action;
    setState(self, mergeObj, actionName);
  } else if (typeof action === 'function') {
    return (function(event) {
      const mergeObj = action(self, event, actionName || action.name);
      setState(self, mergeObj, action.name);
    });
  }
};
