(function() {
  ...
  */
  ...
  function mergeState(space, mergeObj, causedBy, skipNotify) {
    ...
    mergedChildSpaces.forEach(function(childSpace) {
      notifyChange(childSpace, causedBy);
    });
  ...
  */
  ...
  Space.prototype.apply = function apply(callback) {
    var self = this;
    var causedBy;
  ...
  Space.prototype.update = function update(key) {
    var self = this;
    return function updateHandler(value) {
      var causedBy = self.name + '#update' + '-' + key;
  ...
  Space.prototype.setState = function setState(mergeObj, actionName) {
    if (typeof mergeObj === 'function') {
      throw new Error(
        'Do not pass a function to Space#setState. Instead, use Space#apply.'
      );
    }
