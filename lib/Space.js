...
function formState(state) {
  if (state instanceof Space) {
    const space = state;
    return formState(space.state);
  }

  Object.keys(state).forEach(function (key) {
    const item = state[key];
    if (item instanceof Space) {
      state[key] = formState(item.state);
    } else if (Array.isArray(item)) {
      state[key] = item.map(function (item) {
        return formState(item);
      });
    }
  });
  return state;
...
Space.prototype.doAction = function doAction(action) {
  ...
  return (function(event) {
    ...
    Object.assign(self.preState, mergeObj);
    notifyChange(self, actionName);
  });
};
...
Space.prototype.subSpace = function space(nameOrState, id) {
  ...
  if (id) {
    const index = findIndexById(this.preState[name], id);
    subSpace = this.preState[name][index];
    subSpace.name = name + '[' + id + ']';
  } else if (this.preState[name] instanceof Space) {
    subSpace = this.preState[name];
    subSpace.name = name;
  } else {
    subSpace = new Space(this.preState[name], this);
...
Space.prototype.subscribe = function subscribe(subscriber) {
  ...
  subscriber(this, ['initialized']);
};

Space.prototype.getState = function getState() {
  const self = this;
  const state = Object.assign({}, this.preState);
  formState(state); // replaces spaces in the state with their states
