(function() {
  ...
  */
  ...
  function mergeSpace(oldSpace, mergeObj) {
    return new Space(
      Object.assign({}, oldSpace, mergeObj),
      oldSpace._name,
  ...
  function isSimilarSpace(a, b) {
    if (!isSpace(a) || !isSpace(b)) return false;
    if (a === b) return true;
    return (
      a._name === b._name && newestSpace(a._parent) === newestSpace(b._parent)
    );
  }
  ...
  function Space(state, name, parent, oldSpace) {
    ...
    */
    ...
    for (var i = 0; i < keys.length; i++) {
      ...
      var isSimilar = isSimilarSpace(val, prevSpace);
      // If the passed in value is a space, only use it if it belongs here
      // If it's a transplant, convert to object
      if (isSpace(val) && !isSimilar) val = val.toJSON();
