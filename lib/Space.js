(function() {
  ...
  */
  ...
  function merge(mergeObj) {
    this.space = mergeSpace(this.space, mergeObj);
  ...
  function push(newItem) {
    ...
    this.space = new Space(newState, space._name, space._parent, space);
  ...
  function replace(replaceState) {
    this.space = replaceSpace(this.space, replaceState);

    return this.space;
  }

  function unshift(newItem) {
    var space = this.space;
    var newState = space.toJSON().slice();
    newState.unshift(newItem);
    this.space = replaceSpace(space, newState);
  ...
  function getActionParams(space, value, args, causedBy) {
    ...
    actionParams.merge = merge.bind(actionParams);
    actionParams.replace = replace.bind(actionParams);
    actionParams.push = push.bind(actionParams);
    actionParams.remove = remove.bind(actionParams);
    actionParams.unshift = unshift.bind(actionParams);
  ...
  */
  ...
  function Space(state, name, parent, oldSpace) {
    ...
    */
    ...
    if (Space.secretCompatMode) {
      space.state = state;
      space.bindTo = function bindTo(action) {
        var args = [].slice.call(arguments, 1);
        var causedBy = `${space._name}#${action.name}`;
        return function(event) {
          var result = action.apply(null, [space].concat(args).concat(event));
          if (result && result.then) {
            return result.then(function(result) {
              return space(result, causedBy);
            });
          }
          return space(result, causedBy);
        };
      };
      space.applyValue = space;
      space.setState = space;
      space.replaceState = function replaceState(state) {
        replaceAndNotifySubscribers(space, state, 'compatReplaceState');
      };
      space.rootSpace = getRootSpace(space);
      space.getRootSpace = getRootSpace.bind(null, space);
      space.subSpace = function(key) {
        if (isSpaceArray(space)) {
          return space.slice().find(function(item) {
            return item.id === key;
          });
        }
        return space[key] ? space[key] : new Space({}, key, space);
      };
