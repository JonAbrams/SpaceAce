(function() {
  ...
  */
  ...
  Space.prototype.setState = function setState(action, actionName) {
    var self = this;
    var causedBy;
    var nameGuess = new Error().stack
      .match(/at (\S+)/g)[1]
      .slice(3)
      .match(/(?:.*\.)*?[^.]*\.([^.]*)$/);
    nameGuess = nameGuess ? nameGuess[1] : 'unknown';

    if (typeof action === 'object') {
      var mergeObj = action;
      causedBy = self.name + '#' + (actionName || nameGuess);
      mergeState(self, mergeObj, causedBy);
    } else if (typeof action === 'function') {
      return function(event) {
        var mergeObj = action(self, event);
        causedBy = self.name + '#' + (actionName || action.name || nameGuess);
        mergeState(self, mergeObj, causedBy);
      };
    }
  };
